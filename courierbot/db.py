...
class BotDBClass:
    ...
    def user_exists(self,user_bot_id):
        result=self.cursor.execute('select user_bot_id from users_profile where user_bot_id=?',(user_bot_id,)).fetchall()
    ...
    def get_name_user(self, user_bot_id):
        return self.cursor.execute('select name from users_profile where user_bot_id=?', (user_bot_id,)).fetchone()[0]
    def get_user_id(self,user_bot_id):
        return self.cursor.execute('select user_id from users_profile where user_bot_id=?', (user_bot_id,)).fetchone()[0]
    ...
    def get_user_profile(self,user_id):
        return self.cursor.execute('select * from users_profile where user_id=?',(user_id,)).fetchone()
